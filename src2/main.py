...
from src2.features.DiscoreCrawler.discore_crawler import DiscoreCrawler
from src2.features.chrome_controller.chrome_controller import ChromeController
...
class FormFiller(QWidget):
    def __init__(self):
        ...
        self.crawler_driver = (
            None  # 하위 호환용 - discore_crawler.crawler_driver를 가리킴
        )
        self.rows = []
        self.crawling_rows = []
        ...
        self.crawling_select_area.currentIndexChanged.connect(
            self._handle_address_click
        )  # 선택 시그널
        ...
        # disco.re 크롤러 초기화
        self.discore_crawler = DiscoreCrawler(
            gui_ref=self, headless_mode=self.headless_mode
        )
    ...
    def _perform_submission(
        self, row: RowWidget, *, show_popups: bool
    ) -> tuple[bool, str]:
        ...
        if mode == "select" and not input_value.strip():
            ...
            if show_popups:
                QMessageBox.warning(
                    self, "입력 부족", "셀렉트 항목은 내용(선택값)이 필요해!"
                )
    ...
    def load_presets(self, silent: bool = False):
        ...
        try:
            ...
            if not silent:
                QMessageBox.warning(
                    self, "불러오기 실패", f"불러오다 오류가 났어: {exc}"
                )
    ...
    def update_status(self, message: str):
        ...
        if len(self.output_bottom_console_history) > 50:
            self.output_bottom_console_history = self.output_bottom_console_history[
                -50:
            ]
        ...
        if SETTINGS_PATH.exists():
            try:
                with SETTINGS_PATH.open("r", encoding="utf-8") as fp:
                    ...
                    LOGGER.info(
                        "설정 파일 로드 완료: headless_mode=%s", self.headless_mode
                    )
            except (OSError, json.JSONDecodeError) as exc:
    ...
    def _toggle_debug_logging(self):
        ...
        if is_debug:
            ...
            self.update_status(
                "디버그 로깅이 활성화되었어. 더 자세한 로그를 확인할 수 있어."
            )
        else:
    ...
    def add_crawling_row(self, preset=None):
        ...
        if preset:
            self.update_status(
                f"크롤 프리셋 '{preset.get('title', '이름 없음')}' 추가 완료."
            )
        else:
    ...
    def save_crawl_presets(self):
        ...
        for row in self.crawling_rows:
            ...
            entries.append({"title": title, "match_item": match_item})
