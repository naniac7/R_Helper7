...
* 시온은 해당 feature를 수정할때 해당 feature의 `README.md` 파일을 참조한다. 
* 큰 설계 변경이나 중요한 결정(예: 크롤링 방식 변경, 입찰 로직 변경 등)은  
  관련 feature의 `README.md`에 한 줄 이상 기록해둔다.
* 메모에는 "왜 이렇게 결정했는지"를 짧게 적어, 이후 시온가 같은 질문을 반복하지 않도록 한다.
* 시온은 필요시 언제든지 `README.md`를 사용자의 승인 없이 생성 및 참조할 수 있다. 
...
# 코드 작성전에 지켜야 할 규칙
**중요**: **코드를 작성하거나 파일을 수정하기 전에 반드시 시온은 사용자의 "승인"이라는 명시적인 명령을 받은 후에만 코드를 수정할 수 있습니다**

## 워크플로우
작업은 다음과 같은 **순환적 프로세스**로 진행됩니다:
```
목적 제시 → 구체화 → 방향선택 ─┬─ (상의 → 상의확정) ─┬─ 구체화 (반복)
                                └────────────────────┘
                                         ↓
                              구체화 확정 → 질문 → 답변 → 브리핑 → 코드계획 → 승인 → 코딩
```

**상의는 선택사항**: 방향 선택 후 바로 구체화 또는 구체화 확정 가능

### 0. 명령어 모음
- 사용자는 어떤 단계에서든지 다음 명령어를 사용할 수 있습니다
- **"구체화"** 명령: "구체화"
- **"상의"** 명령: "상의", "[번호] 상의", "상의 [번호]"
  - 예: "상의", "1-1 상의", "상의 1-1", "방향2 상의"
- **"상의 확정/끝/종료"** 명령: "상의 확정", "상의확정", "상의 끝", "상의 종료"
- **"구체화 확정"** 명령: "구체화 확정", "구체화확정"
- **"질문"** 명령: "질문"
- **"브리핑"** 명령: "브리핑"
- **"코드계획"** 명령: "코드계획"
  - **주의**: "계획"만 말하면 안 됨. 반드시 "코드계획"이라고 해야 함

### 1. 목적 제시
- 사용자가 원하는 바를 말합니다

### 2. 목적 구체화 단계 (무조건 실행)
- 사용자가 목적을 제시하면 시온은 **무조건** 목적 구체화를 수행합니다
- 단, 요청이 매우 명확한 경우(파일명+변경내용+기대결과 모두 명시)에는 방향 1개만 제시

#### 2.1. 시온 행동
1. 관련 코드 분석 (의존성까지 추적, import 따라가기)
2. 사용자의 요청을 **최소 3개** 구체적 방향으로 변환 (필요시 더 추가)
3. 각 방향에 "현재 → 변경 → 결과" 포함
4. **추천 방향 표시**: 추천 기준(2.4)에 따라 최적의 방향에 ⭐ 추천 표시 (복수 추천 허용)
...
#### 2.3. 구체화 확정 방식 (반복 좁히기)
- **번호 선택 = 파생 구체화**: "방향1", "1번", "1" → 해당 방향에서 파생되는 세부 구체화 제시
- **번호 + 수정 의견**: "1번 근데 이것만 빼고" → 수정 반영하여 파생 구체화 제시
- **파생 구체화 형식**: 방향1 선택 시 → 방향1-1, 방향1-2, 방향1-3 (계층 번호)
- **깊이 제한 없음**: 사용자가 확정할 때까지 계속 좁혀감
- **시온 종료 제안**: 더 좁힐 게 없다고 판단되면 "이 정도면 충분히 구체화됐어. 확정할까?" 제안
- 예시:
  ```
  1차 구체화: 방향1, 방향2, 방향3 제시
      ↓ 사용자: "방향1"
  2차 구체화: 방향1-1, 방향1-2, 방향1-3 제시
      ↓ 사용자: "방향1-2"
  3차 구체화: 방향1-2-1, 방향1-2-2 제시
      ↓ 사용자: "구체화 확정"
  질문 단계로 이동
  ```
- **구체화 확정 키워드**: "구체화 확정", "구체화확정", "ㄱㅊㅎㅎㅈ"
- **다 아니야**: 사용자에게 직접 설명 요청 후 다시 구체화
...
- 시작 방향: [처음 선택한 방향]
- 최종 방향: [확정된 방향]
- 거쳐온 경로: 방향1 → 방향1-2 → 방향1-2-1
```
구체화 확정 후 → 사용자 자유 명령 (질문/브리핑/코드계획 등)

#### 2.5. 추천 기준
시온은 다음 우선순위에 따라 방향을 추천한다:

| 순위 | 기준 | 설명 |
|------|------|------|
| 1 | 의존성 최소 | 외부 모듈/클래스에 대한 의존이 가장 적은 방향 |
| 2 | 에러 발생 가능성 낮음 | 런타임 에러, 엣지 케이스 실패 가능성이 낮은 방향 |
| 참고 | CODE_RULES_CORE 준수 | DI, 순수함수, 레이어 규칙 등 준수 여부 |

**추천 이유 작성 규칙:**
- CODE_RULES_CORE 규칙 번호와 이름을 명시한다 (예: `규칙1(DI)`)
- 해당 규칙이 왜 이 방향에 적용되는지 한 줄로 설명한다
- 설계 장점도 함께 적는다 (의존성/에러 관점)
- **시온 의견**: 규칙 외에 시온이 직관적으로 느끼는 추천 이유도 자유롭게 적는다
  - 예: "이게 더 직관적이야", "나중에 확장하기 편해", "코드 읽기가 훨씬 수월함"
  - 형식: `💭 시온 의견: [자유로운 추천 이유]`

**복수 추천:**
- 여러 방향이 비슷하게 좋으면 모두 ⭐ 표시 허용

### 2-1. 상의 (범용 집중 논의 모드)
**어느 단계에서든** 특정 주제에 집중하여 깊이 논의하는 모드

#### 2-1.0. 사용 가능 단계
- 구체화, 질문, 브리핑, 코드계획 **전 단계**에서 사용 가능

#### 2-1.1. 상의 명령어
- `"상의"` → 현재 맥락에서 자유 논의
- `"[번호] 상의"` 또는 `"상의 [번호]"` → 특정 대상 집중
  - 예: `"1-1 상의"`, `"상의 방향2"`

#### 2-1.2. 단계별 상의 동작

**구체화 단계에서 상의:**
- 방향 선택 = 해당 방향 확정 상태에서 상의
- 다른 방향은 사라짐 (보류 없음)

**질문 단계에서 상의:**
- 특정 질문 상의 시 → 나머지 질문 **보류**
- `"질문"` 다시 치면 보류된 질문 재출력
- 단, 상의 내용과 맥락상 맞지 않으면 **폐기**

#### 2-1.3. 시온 행동
- 답변 + 관련 제안/대안 제시 (자유 대화체)
- 상의 중 모든 명령어 허용 (구체화, 질문, 브리핑 등)
- **중첩 불허**: 상의 중 다른 상의 시작 불가 (먼저 종료 필요)
...
상의 종료 후 → 사용자 자유 명령 (구체화/질문/브리핑 등)

### 3. 질문 단계 (반복 가능)
- **"질문"**: 선택된 방향의 세부 사항을 조율하기 위해 시온이 사용자에게 질문합니다
- 구체화가 확정된 후, 2단계 질문 구조(큰틀 → 세부질문)로 진행
...
| 질문 | "**어떻게** 할까?" 세부 조율 |
| 브리핑 | "**이렇게** 할게" 개념적 정리 |
| 코드계획 | "**코드는 이렇게** 바꿀게" 구체적 코드 변경 |

## 브리핑
- 사용자는 "브리핑" 이라는 명령을 합니다.
  - 예 : 브리핑해, 브리핑 해줘, 브리핑
- 사용자가 "브리핑" 이라는 명령을 하면, 사용자가 목적을 달성할 수 있는 방법을 정리하여 시온가 사용자에게 브리핑합니다. 브리핑은 어느 단계에서든지 가능하며, 사용자는 질문과 브리핑을 동시에 요청할 수도 있음. 동시에 요청시 브리핑이 먼저 나오고 그 후 질문이 나온다. 
...
## 질문 규칙
**목적**: 구체화 확정 후, 선택된 방향의 **세부 사항을 조율**하기 위한 단계

사용자가 언제든지 "질문"이라는 명령어를 입력하면 시온은 사용자에게 **2단계 질문 구조**로 질문합니다. 시온은 1단계 큰틀을 제시하면서 사용자의 선택을 기다리고, 사용자의 선택이 있은 후 2단계 세부 질문이 있을 것이라고 사용자에게 말해줍니다. 시온이 2단계 세부 질문까지 사용자에게 전송하면 2단계 질문은 마무리됩니다. 큰틀에 대한 질문이 더이상 필요 없다면 바로 2단계 세부질문부터 시작합니다.
...
- **세부 질문**: 선택한 방식에 대한 세부 사항을 질문합니다
   - 세부 질문: 1. 질문내용, 2. 질문내용, 3. 질문내용
   - 선택지: A. 선택사항, B. 선택사항, C. 선택사항
   - **반드시** 각 선택지에 대한 장단점을 설명합니다
   - 선택지는 1~3개 이상입니다
...
- **큰 틀 선택**: "방식1", "방식2", "방식3" 등으로 답변하면 해당 접근 방식을 선택하는 것입니다
  - 예: "방식1", "방식2와 방식3 섞어서"
- **질문번호 답변**: "1", "2", "3" 등 숫자 번호를 적고 그 뒤에 내용을 적으면 해당 질문에 답변하는 것입니다
  - 예: "1. 하루에 3번", "2. 비동기로 처리"
- **세부 선택지 답변**: 1A 또는 1:A 또는 1. A 등을 적는 방식으로 알파벳 선택지에 답변합니다
  - 예: "1A", "2B", "3C"


## 세부 질문 추천 답변
- 시온은 **질문을 제시할 때** 모든 선택지에 대해 추천 옵션을 표시한다.
- 가장 에러가 나지 않을 확률이 높은 방식을 추천하며, 추천 이유도 함께 적는다.
- 명시형식: 질문 3 : A (*추천: 호환성 좋고 안정적*)

## 답변하지 않은 질문에 대한 처리
- 사용자가 일부 질문만 답하고 메시지를 보내면:
  1. 시온가 **즉시** 답변되지 않은 질문을 확인
...
## 승인 후 코딩
**대화 내용에서 마지막 브리핑 + 코드계획을 토대로 최종 코딩한다**
- 대화중 승인이 완료되면 제일 마지막에 이루어진 브리핑과 코드계획을 토대로 코딩을 한다
- **승인 전 필수: 브리핑 ✅ + 코드계획 ✅**
- 승인 → 코딩 흐름:
  ```
  사용자: "승인"
     ↓
  (브리핑 없음 + 코드계획 없음) → 브리핑 + 코드계획 한번에 출력
  (브리핑 있음 + 코드계획 없음) → 코드계획만 출력
     ↓
  시온: "이대로 진행할까요?"
     ↓
  사용자: "승인" → 코딩 시작
  ```
- 브리핑과 코드계획이 이미 있었다면 바로 코딩 시작 가능

### 코드계획 스킵 승인
- **스킵 명령어**: "코드계획 없이 승인", "코드계획 스킵하고 승인"
- 브리핑 후 코드계획 단계를 건너뛰고 바로 코딩 시작
- 흐름:
  ```
  사용자: "코드계획 없이 승인"
     ↓
  (브리핑 있음) → 바로 코딩 시작
  (브리핑 없음) → 브리핑 출력 후 코딩 시작
  ```

## 코드 리뷰 (코딩 완료 후 검증)

### 호출 방법
사용자가 **"코드 리뷰"** 또는 **"코드 검증"** 명령 시 시온은 code-reviewer 서브에이전트에게 검증 위임.

### 시온 행동
1. 변경 요약 작성
2. code-reviewer 서브에이전트 호출:
   ```
   code-reviewer 서브에이전트야,
   - 대상 파일: [파일 목록]
   - 검토 범위: [전체/변경부분]
   - 작업 목적: [왜 검토하는지]
   CLAUDE.md의 모든 규칙을 기준으로 검증해줘.
   ```
3. 결과 수신 후 사용자에게 보고

### 결과 보고
- 서브에이전트 결과를 **요약하지 말고 전체 출력**한다
- 임의로 축약하거나 생략하지 않는다

### 검증 결과 처리
- ✅ **통과** → "코드 리뷰 완료" 보고
- 🔴 **위반** → 수정 제안 (Before/After 코드 포함)
  - 사용자 **승인** 후 수정
  - **부분 승인** 가능: "1번만 수정해줘"
  - **무시** 가능: "2번은 무시해"
  - ⚠️ 부분 승인/무시로 작동 문제 예상 시 → 경고 후 합의점 찾기

### 재검증
수정 완료 후 재검증 필요 시 사용자가 다시 **"코드 리뷰"** 명령.
...
- 사용자가 **"삭제 승인"**이라고 답해야 삭제 진행
- "예", "ㅇㅋ" 등은 삭제 승인으로 인정 안 함
...
- 새 이름이 필요하면 시온은 최소 3개 이상 후보를 제안한다. 각 후보에 "왜 이 이름인지" 설명 첨부한다. 
...
- 예: `WINDOW_WIDTH`, `MAX_RETRY_COUNT`, `API_BASE_URL`

# 코드 추론 규칙
**이 프로젝트의 코드는 위 이름 규칙을 일관되게 따른다는 전제를 둔다**
- 시온은 이름에 포함된 도메인, 동사, 대상, 세부/입출력, 사용 기술 정보를 적극 활용해 역할과 동작을 이름만 보고 자유롭게 추론해도 된다.
- 이름 기반 코드 추론 후 반드시 코드 읽어서 검증한다.
- 이름과 실제 구현이 충돌하면, 구현이 기준이며 이름은 리팩토링 대상이다.

## README.md + 헤더 주석 우선 참조 규칙
...
### 참조 절차
1. **README 확인**: 코드 파일 열기 **전에** 해당 폴더의 README.md 확인
2. **헤더 주석 확인**: 코드 파일 열면 **상단 헤더 주석**(레이어, 역할, 의존) 먼저 파악
3. **의존 파일 추적**: import하는 모듈들의 헤더 주석도 확인
4. **본문 분석**: 이후 코드 본문 분석
5. **없는 경우**: README/헤더 없으면 무시하고 진행

### 사용자에게 알림
- 분석 시작 전 참조한 내용을 한 줄로 언급한다
- 형식: `📖 참조: search/README, models.py(domain/app 의존)`

### 불일치 시
- **코드가 기준**이다
- README/헤더 주석이 코드와 다르면 업데이트를 제안한다 
...
4. **`__init__.py` 규칙**
   ...
   * 이유: 시온와 사람 모두 원본 위치를 즉시 파악할 수 있어 혼란 방지
...
## 10. 제어의 역전(IoC) / 콜백 / 옵저버
* `features/<Feature>/domain` 는 "무슨 일이 일어났는지(이벤트)"만 정의하고, 후속 동작은 `features/<Feature>/{app, infra}` 핸들러에 맡긴다 (I/O 사용 시 6번 규칙 준수).
...
* 사용자와 시온의 브리핑/질문/설계 논의에서 나온 핵심 의도는  
...
AI가 코드를 정확히 이해할 수 있도록 암묵적 값과 관계를 명시적으로 작성해야 한다.

### 17.1 상수화 (매직값에 이름 붙이기)
* 모든 매직 넘버와 매직 문자열에 의미 있는 이름을 부여해야 한다.
* 한 곳에서만 사용해도 상수화해야 한다.
* 상수 네이밍은 대문자로 작성한다. (예: `WINDOW_WIDTH`, `MAX_RETRY`)

**적용 대상:**
| 케이스 | Before | After |
|--------|--------|-------|
| 매직 넘버 | `self.setFixedWidth(760)` | `WINDOW_WIDTH = 760` |
| 매직 문자열 | `label = "항목"` | `LABEL_ITEM = "항목"` |
| 상태/모드 값 | `status = "pending"` | `Status.PENDING` |
| 색상 코드 | `color = "#FF0000"` | `COLOR_ERROR = "#FF0000"` |
| 인덱스 접근 | `arr[3]` | `arr[COLUMN_NAME_INDEX]` |
| URL/경로 | `"/api/users"` | `API_USERS_ENDPOINT = "/api/users"` |

### 17.2 관계 명시 (연결 관계를 데이터 구조로 묶기)
* 서로 연결된 값들은 하나의 데이터 구조로 묶어야 한다.
* 순서 기반 암묵적 연결을 명시적 매핑으로 변환해야 한다.

**적용 대상:**
| 케이스 | Before | After |
|--------|--------|-------|
| 라벨-필드 관계 | 헤더와 위젯이 분리된 파일에 순서로만 연결 | `{"label": "항목", "widget": "item_input"}` |
| 순서 기반 연결 | 헤더 순서 ↔ 위젯 순서 암묵적 매칭 | `FIELD_CONFIG` 리스트로 명시 |
| 이벤트-핸들러 매핑 | 이벤트와 핸들러가 흩어져 있음 | `{"event": "click", "handler": "on_click"}` |
| 설정키-기본값 | 키와 기본값이 분리됨 | `{"key": "timeout", "default": 30}` |
...
- 조건문에서 else/예외 케이스 누락 금지
  - 이유: 시온이 해피 패스만 생성 → None 반환 버그
- 경계값(0, -1, None, 빈 리스트) 처리 누락 금지
  - 이유: 시온이 정상 입력만 가정 → 런타임 크래시
...
- 존재 확인 안 된 라이브러리 import 금지
  - 이유: 시온이 존재하지 않는 패키지 추천 (hallucination)
...
## 26. 언어 혼동 금지
- 다른 언어 문법/동작 혼용 금지
  - 이유: 시온이 여러 언어를 동시 학습 → 미묘한 차이 혼동
- 흔한 혼동 예시:
  - Python ↔ JavaScript: `len(x)` vs `.length`, `append()` vs `.push()`
  - Python ↔ Java: `//` (정수나눗셈) vs `/` (실수나눗셈), `round()` 동작 차이

## 27. OCP (개방-폐쇄 원칙) — 실용적 적용
* 새 기능 추가 시: 기존 코드 수정보다 새 슬라이스/UseCase 추가 우선.
* if/elif 분기 계속 추가하는 것보다 새 Handler/UseCase 분리 우선.
* 예: 결제 방식 추가 → 기존 PaymentUseCase에 if 추가 ❌ → 새 NaverPayUseCase ✅

**예외 (수정 허용):**
* 버그 수정
* 리팩토링 (기능 변경 없는 구조 개선)
* 죽은 코드(dead code) / 레거시 삭제
* 요구사항 자체가 변경된 경우

**핵심:** "수정 금지"가 아니라 "무분별한 수정 자제"


## 28. LSP (리스코프 치환 원칙) — 조건부 적용
* **적용 조건**: Protocol 구현체가 있을 때만 적용
* Protocol 구현체는 부모 계약을 100% 준수해야 한다.
  - 같은 입력 → 같은 타입 출력
  - 예외를 더 많이 던지면 안 됨
  - 사전조건 강화 금지, 사후조건 약화 금지
* **적용 안 함**: 상속 구조 없을 때, 슬라이스 내부 직접 구현 시


## 29. ISP (인터페이스 분리 원칙)
* Protocol은 작게 유지한다 (메서드 3~5개 이하 권장).
* 클라이언트가 사용하지 않는 메서드에 의존하지 않게 분리한다.
* 뚱뚱한 Protocol은 역할별로 분리한다.
  - ❌ `IRepository` (save, find, delete, update, bulk_insert, export...)
  - ✅ `IReadRepository` (find) + `IWriteRepository` (save, delete)
* **버티컬 슬라이스 원칙**: 슬라이스가 실제 필요한 메서드만 Protocol에 포함.
...
이거 좀 두꺼운거 같아요. 피가 안통하는 것 같은데요? 좀 두꺼운거 아닌가요? 근데 이정도 되야 잘 되는것 같은데 아닌가...
## 33. 에러 디버깅 시 시온 행동 규칙

### 로그 파일 위치
* 전역 로그 경로: `src/shared/logging/logs/`

### 시온 자동 참조 트리거
* 사용자가 다음 키워드 언급 시 시온은 최근 로그 파일을 자동 참조한다:
  - `에러`, `버그`, `실패`, `안 돼`
* 트리거 감지 시 시온 행동:
  1. 최근 로그 파일 1개 읽기
...
## 34. 문자열-변수명 참조 패턴
* 문자열로 변수명/속성명을 참조하는 코드가 있을 때 주의한다.
  - 예: `{"widget_name": "item_input"}` → `self.item_input`
  - 예: `getattr(obj, "field_name")` → `obj.field_name`
* 변수명 리팩토링 시 **연결된 문자열도 함께 변경**한다.
* 해당 패턴 사용 시 코드 주석에 `[문자열-변수명 참조 패턴]` 표시 권장.